...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             * Basically, implementation of this method must perform the logic of the following code "atomically":
             */
            val lowerInd = min(fromIndex, toIndex)
            val higherInd = max(fromIndex, toIndex)

            val acquiredLower = acquire(lowerInd, this)
            val acquiredHigher = acquire(higherInd, this)

            if (acquiredLower != null && acquiredHigher != null) {
                val from : AcquiredAccount
                val to : AcquiredAccount
                if (lowerInd == fromIndex) {
                     from = acquiredLower
                     to = acquiredHigher
                } else {
                     from = acquiredHigher
            ...
            release(higherInd, this);
            release(lowerInd, this);
        }
    }
}